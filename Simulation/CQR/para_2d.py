...
def process_combination(setting, n, conf_methods_list, ratio_train, quantiles_net, verbose, significance, workgrid_x):
    ...
    Test_X1 = pyreadr.read_r(f"Data/Setting_2d_{setting}_n_{n}_Test_X1.RData")["Testing_X1"]
    Test_X2 = pyreadr.read_r(f"Data/Setting_2d_{setting}_n_{n}_Test_X2.RData")["Testing_X2"]
    Test_Y = pyreadr.read_r(f"Data/Setting_2d_{setting}_n_{n}_Test_Y.RData")["Testing_Y"]
    Train_X1 = pyreadr.read_r(f"Data/Setting_2d_{setting}_n_{n}_Train_X1.RData")["Training_X1"]
    Train_X2 = pyreadr.read_r(f"Data/Setting_2d_{setting}_n_{n}_Train_X2.RData")["Training_X2"]
    Train_Y = pyreadr.read_r(f"Data/Setting_2d_{setting}_n_{n}_Train_Y.RData")["Training_Y"]
    
    # Convert to numpy arrays
    Train_X1 = np.asarray(Train_X1).astype(np.float32)
    Train_X2 = np.asarray(Train_X2).astype(np.float32)
    Train_Y = np.asarray(Train_Y).astype(np.float32)
    Test_X1 = np.asarray(Test_X1).astype(np.float32)
    Test_X2 = np.asarray(Test_X2).astype(np.float32)
    ...
    for r in range(200):
        X_train1 = Train_X1[r, :].reshape(-1, 1)
        X_train2 = Train_X2[r, :].reshape(-1, 1)
        X_train = np.hstack((X_train1, X_train2))
        y_train = Train_Y[r, :]
        X_test1 = Test_X1[r, :].reshape(-1, 1)
        X_test2 = Test_X2[r, :].reshape(-1, 1)
        ...
        for i in range(3):
            ...
            Cov[i][r, :] = compute_proportions(X_test1.reshape(-1), covered, workgrid_x)
            Len[i][r, :] = compute_means(X_test1.reshape(-1), widths, workgrid_x)
    ...
    output_file = f"results/Setting_2d_{setting}_n_{n}.json"
