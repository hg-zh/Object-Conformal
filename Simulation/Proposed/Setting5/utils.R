...
Get_F<-function(x,a,b){#interplotation
  return(approx(a,b,x,rule=2)$y)
}
Otime<-function(a,T0,x) {
  if(a==0){return(x)}
  else if(a>0){
    if(a<=1){return(x+a*(T0-x))}
    else{
      b=floor(a)
      temp<-x
      for (i in 1:b) {
        temp<-Get_F(temp,x,T0)
      }
      return(temp+(a-b)*(Get_F(temp,x,T0)-temp))
    }
  } else{
    T_in<-Get_Inverse(x,T0)
    if(a>=-1){return(x+a*(x-T_in$y))}
    else{
      b=floor(abs(a))
      temp<-x
      for (i in 1:b) {
        temp<-Get_F(temp,x,T_in)
      }
      return(temp+(abs(a)-b)*(Get_F(temp,x,T_in)-temp))
    }
  }
}
...
belongs_to_W2<-function(datapoint,input_set,tolerance){
  for (i in 1:length(input_set)) {
    if(met_W2(datapoint,input_set[[i]] )<=tolerance){
      return(TRUE)
    }
  }
  return(FALSE)
